...
from services.telegram import tg_send_message, tg_answer_callback, notify_operator
        ...
        for i in range(0, len(time_options), 2):
            ...
            for j in range(2):
                if i + j < len(time_options):
                    time = time_options[i + j]
                    row.append({"text": f"🕒 {time}", "callback_data": f"delivery_time_{time}"})
            keyboard_buttons.append(row)
        
        keyboard_buttons.append([{"text": "📝 Вказати свій час", "callback_data": "custom_delivery_time"}])
