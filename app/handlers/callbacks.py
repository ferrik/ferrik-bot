...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await query.edit_message_text("⚠️ Виникла помилка. Спробуйте ще раз.")
        except:
            pass
...
async def handle_category_callback(query, context, data):
    ...
    items = {
        "pizza": [
            {"id": 1, "name": "Маргарита", "price": 180},
            {"id": 2, "name": "Пепероні", "price": 200},
            {"id": 3, "name": "4 Сири", "price": 220},
            {"id": 4, "name": "М'ясна", "price": 240}
        ],
        "burgers": [
            {"id": 5, "name": "Класичний", "price": 150},
            {"id": 6, "name": "Чізбургер", "price": 170},
            {"id": 7, "name": "Бекон бургер", "price": 190}
        ],
        "snacks": [
            {"id": 8, "name": "Картопля фрі", "price": 60},
            {"id": 9, "name": "Нагетси", "price": 80},
            {"id": 10, "name": "Крильця", "price": 120}
        ],
        "drinks": [
            {"id": 11, "name": "Coca-Cola", "price": 40},
            {"id": 12, "name": "Sprite", "price": 40},
            {"id": 13, "name": "Сік", "price": 50}
        ]
    }
    
    category_items = items.get(category, [])
    
    message = f"🍕 <b>Категорія: {category.title()}</b>\n\n"
    message += "Обери товар:\n\n"
    
    keyboard = []
    for item in category_items:
        message += f"▪️ {item['name']} - {item['price']} грн\n"
        keyboard.append([
...
async def handle_add_item_callback(query, context, data):
    ...
    if item:
        ...
        await query.answer(f"✅ {item['name']} додано в кошик!", show_alert=True)
    else:
